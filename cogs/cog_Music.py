...
# Import main modules
import discord, asyncio, pafy, random, requests, re
...
class Player(commands.Cog):
    ...
    async def search_(self, amount, song, get_url=False):
        ...
        info = await self.bot.loop.run_in_executor(None, lambda: YoutubeDL({"format": "bestaudio", "quiet": True, "noplaylist": True, "audioquality": "2"}).extract_info(f'ytsearch{amount}:{song}', download = False, ie_key = 'YoutubeSearch'))
